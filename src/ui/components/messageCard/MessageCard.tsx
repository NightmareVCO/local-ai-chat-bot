...
export type MessageCardProperties = React.HTMLAttributes<HTMLDivElement> & {
  avatar?: string;
...
const MessageCard = React.forwardRef<HTMLDivElement, MessageCardProperties>(
  (
    {
      avatar,
      message,
    ...
    const messageReference = React.useRef<HTMLDivElement>(null);
    ...
    const failedMessage = (
      <p>
        ...
        our help center at&nbsp;
        <Link href="mailto:vladimircuriel@outlook.com" size="sm">
          vladimircuriel@outlook
        </Link>
      </p>
    );
    ...
    const handleCopy = React.useCallback(() => {
      ...
      const valueToCopy =
        stringValue || messageReference.current?.textContent || "";
        ...
        <div className="flex flex-col w-full gap-4">
          ...
          >
            <div
              ...
              className={"pr-20 text-small"}
              dangerouslySetInnerHTML={{
                __html: hasFailed ? failedMessage : (message as TrustedHTML),
              }}
            ></div>
            {showFeedback && !hasFailed && (
              <div className="absolute flex rounded-full right-2 top-2 bg-content2 shadow-small">
                <Button
                  isIconOnly
                  radius="full"
                  size="sm"
            ...
            {attempts > 1 && !hasFailed && (
              <div className="flex items-center justify-end w-full">
                ...
                <p className="px-1 font-medium text-tiny text-default-500">
                  {currentAttempt}/{attempts}
                </p>
              </div>
            )}
          </div>
          {showFeedback && attempts > 3 && (
            <div className="flex items-center justify-between px-4 py-3 rounded-medium border-small border-default-100 shadow-small">
